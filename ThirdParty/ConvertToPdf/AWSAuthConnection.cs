...
namespace com.amazon.s3
{
    ...
    public class AWSAuthConnection
    {
        ...
        public PutObjectResult put(string bucket, string key, byte[] obj, MetadataEntry[] metadata, Grant[] accessControlList)
        {
            DateTime timestamp = AWSDateFormatter.GetCurrentTimeResolvedToMillis();
            string signature = makeSignature("PutObjectInline", timestamp);
            ASCIIEncoding ae = new ASCIIEncoding();
            return s3.PutObjectInline(bucket, key, metadata, obj/*ae.GetBytes( obj )*/, obj.Length, accessControlList,
