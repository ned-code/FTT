    ...
    '<div id="toolbar_panel" class="ui-widget">'+
        ...
        '<div id="toolbar_buttons_container">'+
            ...
            '<form id="toolbar_panel_createlink_block" class=" ui-menu  ui-widget-content ui-corner-all " onsubmit="return false;">'+
              '<div><label id="toolbar_panel_button_createlink_text_label">Text</label><input type="text" id="toolbar_panel_button_createlink_text" class="ui-corner-all"></div>'+
              '<div><label id="toolbar_panel_button_createlink_link_label">Link</label><input type="text" id="toolbar_panel_button_createlink_link" class="ui-corner-all" value="http://"></div>'+
              '<div>'+
                '<label>Type</label>'+
    ...
    jQuery('#toolbar_panel_button_hiliteColor_arrow').addClass('ui-corner-right').removeClass('ui-corner-all');   
      ...
      _create: function() {
        ...
        var select = this.element.hide();
				...
				.insertAfter(select)
				.addClass("ui-widget ui-widget-content ui-corner-left ui_custom_input")
        .css('width',select.css('width')); 
        
        self.input = input;
        if(!self.options.isEditable){
          self.input.bind('keypress', function(){return false;});
          self.input.bind('keyup', function(){return false;});
          self.input.bind('keydown', function(){return false;});
        } else {
            self.input.focus(function(){this.select();});
        ...
        var button = jQuery("<button>&nbsp;</button>")
				...
				.addClass("toolbar-custom-combobox-button ui-corner-right ui-button-icon ui-button-icon-only ui_custom_button")
				.position({
					my: "left center",
					at: "right center",
					of: input,
            ...
            var dropdownConteiner = jQuery("<div class='ui_dropdown_container'  id='"+id+"'><div>")
            .insertAfter(input)
            .css('min-width',select.css('width').split('px')[0]*1+button.css('width').split('px')[0]*1);
