...
WebDoc.ImagesUploader = $.klass({
  initialize: function(uploadControlId, imagesLibrary) {
    ...
    .bind('swfuploadLoaded', function(event){
      ...
      this.uploadButtonFlash = this.uploadControl.find('object');
    }.pBind(this))
    .bind('fileQueued', function(event, file){
      ...
      $.swfupload.getInstance(this.uploadControl).setPostParams({
        sessionKeyName : WebDoc.authData.cookiesSessionKeyName,
        "authenticity_token": WebDoc.authData.authToken,
        "image[uuid]": (new MTools.UUID().id)
      });
  ...
  setUploadingUI: function() {
    this.spinner.show();
    this.uploadButtonFlash.css({'visibility':'hidden'}); // do NOT use hide(), or swfuplaod won't work
