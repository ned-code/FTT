function MemberProfileEdit(parent){
	var self = this;
	...
	this.defaults = {
		clickIt: true,  
		closeTipBtn: 'jmb-dtpe-tooltip-button', 
		toolTipClass: 'jmb-dtpe-tooltip',  
		tipContent:'content',
		fixed: false,       
		comparisonObj: false,
		inSpeed: 400,                    
	...
	if(jQuery('iframe[id="jmb_media_iframe"]').length == 0) jQuery(document.body).append(iframe);
...
MemberProfileEdit.prototype = {
	_ajax:function(func, params, callback){
		host.callMethod("member_profile_edit", "JMBMemberProfileEdit", func, params, function(res){
				callback(res);
		})
	},
	setTabs:function(p){
	...
	add:function(p){
		...
		var html = '<div>';
			html += '<fieldset>';
				...
				html += '<table>';
					...
					html += '</tr>';
					html += '<tr>';
						html += '<td><div class="jmb-dtpe-tabs-button save">Save</div></td>';
		...
		jQuery(obj).find('.jmb-dtpe-tabs-button.save').unbind().click(function(){
			if(!self.buttonEvent){
				...
				if(!self.saveActore){
					...
					self._ajax('addRelative', request, function(res){
						self.relative(obj, res);
					});
				} else {
					request = '{"ownerID":"'+self.saveActore.Id+'","FirstName":"'+firstName+'","LastName":"'+lastName+'","Gender":"'+gender+'","Type":"'+type+'"}';
					self._ajax('updateRelative', request, function(res){
						self.relative(obj, res);
					});
				}
			}
	...
	relative:function(obj, res){
		var self = this;
		jQuery(obj).find('.jmb-dtpe-tabs-button-info').html('<span class="loaded">Information sent.</span>');
		jQuery(obj).find('.jmb-dtpe-tabs-button-info span')
		.fadeOut(3000)
		.html('<div class="jmb-dtpe-tabs-button new">New</div>')
		.fadeIn(1000)
		.unbind()
		.click(function(){
			if(confirm('Do you really want to create a new record?')){
				jQuery(obj).find('input[name="FirstName"]').val('');
				jQuery(obj).find('input[name="LastName"]').val('');
				jQuery(this).unbind().remove();
				self.saveActore = null;
			}
		});
		self.saveActore = jQuery.parseJSON(res.responseText);
		self.treeReload();
	...
	getDate:function(ev){
		var day,month,year;
		day = (ev.Day && ev.Day != '00')?ev.Day:false;
		month = (ev.Month && ev.Month != '00')?this.months[ev.Month-1]:false;
		year  = (ev.Year)?ev.Year:false;
		if(day && month && year){
	...
	getPlace:function(obj){
		var placeString = '';
		for(var k in obj){
			placeString += obj[k].Name
			placeString += ',';
		}
		return placeString.substr(0,placeString.length-1);
	},
	getBirth:function(ind){
		if(ind.Birth){
			var b = ind.Birth;
			var place = this.getPlace(b.Place.Hierarchy);
			var date = this.getDate(b);
	...
	getMarried:function(){
		var fam = this.person.fam;
		if(fam == null) return 'edit';
		if(fam.Marriage){
			var marr = fam.Marriage;
			var date = this.getDate(marr);
			var place = this.getPlace(marr.Place.Hierarchy);
			return date + ((place)?' in '+place :'');
		}
		else{
			return 'edit';
	...
	setData:function(obj){
		...
		self.setTextInEditDiv(obj, 'MiddleName', self.person.ind.MiddleName);
		self.setTextInEditDiv(obj, 'LastName', self.person.ind.LastName);
		self.setTextInEditDiv(obj, 'Nick', self.person.ind.Nick);
	...
	details:function(p){
		...
		var html = '<div>';
			html += '<fieldset>';
				...
				html += '<div class="jmb-dtpe-tabs-fieldset-body"><table>';
					...
					html += '</tr>';
					html += '<tr>';
						html += '<td><div class="jmb-dtpe-tabs-input-name"><span>Middle Name:</span></div></td>';
						html += '<td>'+self.getEditDiv('MiddleName')+'</td>';
					html += '</tr>';
					html += '<tr>';
						html += '<td><div class="jmb-dtpe-tabs-input-name"><span>Last Name:</span></div></td>';
					...
					html += '</tr>';
					html += '<tr>';
						html += '<td><div class="jmb-dtpe-tabs-input-name"><span>Know as:</span></div></td>';
						html += '<td>'+self.getEditDiv('Nick')+'</td>';
					html += '</tr>';
					html += '<tr>';
						html += '<td><div class="jmb-dtpe-tabs-input-name"><span>Gender:</span></div></td>';
		...
		jQuery(obj).find('div.jmb-dtpe-source-text').each(function(i,e){
			jQuery(e).unbind().click(function(){
		...
		jQuery(obj).find('div.jmb-dtpe-source-add').unbind().click(function(){
			...
			jQuery(row).find('input.jmb-dtpe-tab-input-submit').unbind().click(function(){
				self.clickRowHandleComplete(row);
			});
			jQuery(row).find('div.jmb-dtpe-source-text').unbind().click(function(){
				self.clickRowHandle(this);
			})
		});
		//switch radio input
	...
	basic:function(obj, type){
		...
		var textDiv = obj;
		jQuery(textDiv).hide();
		jQuery(inputDiv).show().find('input').focus().unbind()
		.focusout(function(){
	...
	events:function(obj, type){
		...
		jQuery(inputDiv).find('input[type="submit"]').unbind().click(function(){
	...
	getMedia:function(){
		...
		var li = jQuery('<li><img id="'+element.Id+'" src="'+element.FilePath+'"></li>');
			jQuery(ul).append(li);
			jQuery(li).unbind().click(function(){
	...
	media:function(p){
		...
		//buttons
		jQuery(obj).find('input[type="button"][name="add"]').unbind().click(function(){
			if(jQuery(obj).find('input[type="file"][name="upload"]').val() != ''){
				jQuery(obj).find('#jmb_media_upload').submit();
			}
		});
		jQuery(obj).find('input[type="button"][name="delete"]').unbind().click(function(){
			if(self.activePhoto == null) return;
		...
		jQuery(obj).find('span[name="save"]').unbind().click(function(){
			self.saveInfo();
		});
	...
	setModal:function(param){
		...
		if(param){
			...
			jQuery(self.modalDiv).remove();
			self.modalDiv = null;
		}
	},
	render:function(obj, container, json){
		var self = this;
		if(typeof(json) == 'object'){
			self.person = json;
			self.createBody(container);
		} else if(typeof(json) == 'string'){
			self._ajax('getPersonInfoJSON', json, function(response){
				self.person = jQuery.parseJSON(response.responseText);
				self.createBody(container);
			});
		} else {
			return false;
	...
	//load:function(button, json, callback){
	load:function(params, callback){
		var self = this;
		var settings = jQuery.extend({}, self.defaults, params);
		jQuery(settings.button).unbind().aToolTip({
			clickIt: settings.clickIt,  
			closeTipBtn: settings.closeTipBtn, 
			toolTipClass: settings.toolTipClass,  
			tipContent:settings.tipContent,
			fixed: settings.fixed,     
			comparisonObj: settings.comparisonObj,
			inSpeed: settings.inSpeed,                    
			outSpeed: settings.outSpeed,                     
			xOffset: settings.xOffset, 
			yOffset: settings.yOffset,
			eventClick:function(body){
				if(settings.hideObject != null) settings.hideObject.hide();
				self.render(this, body, settings.json);
				self.setModal(true);
			},
			eventClose:function(){
				callback(self);
