 ...
 * @package PhpGedView
 * @author Joel A. Bruce
 * @version $Id: class_event.php 6481 2009-11-28 20:17:25Z fisharebest $
 *
 */
...
//	var $sourceCitations = array(); //[0..*]: gSourceCitation
//	var $multimediaLinks = array(); //[0..*]: MultimediaLink
	 ...
	 * @return gEvent
	 */
	function gEvent($subrecord, $lineNumber=-1) {
		global $factarray;
	...
	/**
	 * The gPerson/gFamily record where this gEvent came from
	 *
	 * @return GedcomRecord
	 */
...
//		print "[".$a->getTag().":".$adate->isOK().":".$adate->MinJD()."-".$adate->MaxJD()." ".$b->getTag().":".$bdate->isOK().":".$bdate->MinJD()."-".$bdate->MaxJD()." ".$ret."] ";
	...
	static function CompareType(&$a, &$b) {
		...
		if ($a->getgFamilyId() && $b->getgFamilyId() && $a->getgFamilyId()!=$b->getgFamilyId()) {
		...
		// gEvents not in the above list get mapped onto one that is.
