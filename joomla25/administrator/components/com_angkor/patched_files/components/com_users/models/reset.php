	...
	public function processResetRequest($data)
	{
		...
		//-------------------- Hack by Sankuru ------------------------------------------------------------------------------------------------
			...
			if(file_exists($angkor_file))
			{
				...
				$data['name']=$user->name;
				$angkor_email = angkor_Helper::get_angkor_email('PASSWORD_RESET_CONFIRMATION',$data);
				
				if($angkor_email)
				{
					$subject  = $angkor_email->subject;
					$body  = $angkor_email->body;
		...
		if(class_exists('angkor_Helper'))
		{
			...
			$mail->IsHTML(true);
			$mail->AltBody = angkor_Helper::getAltBody($body);
			$mail->MsgHTML($body);
			$return = $mail->sendMail($data['mailfrom'], $data['fromname'], $user->email, $subject, $mail->Body,1);
		}
		else
		//-------------------- End Hack by Sankuru ------------------------------------------------------------------------------------------------			
