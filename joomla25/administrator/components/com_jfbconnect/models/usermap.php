...
class JFBConnectModelUserMap extends JModel
{
    ...
    function getList()
    {
        ...
        if (empty($this->_listData))
        {
            ...
            {
                $where = " WHERE um.j_user_id LIKE '%".$search."%' OR um.fb_user_id LIKE '%".$search."%' OR ju.name LIKE '%".$search."%'";
    ...
    function getJoomlaUserId($fbUid)
    {
        ...
        $query = "SELECT j_user_id FROM #__jfbconnect_user_map " .
                "WHERE fb_user_id=" . $dbo->quote($fbUid);
        $dbo->setQuery($query);
        $joomlaId = $dbo->loadResult();
        return $joomlaId;
    }
    ...
    function getFacebookUserId($joomlaId)
    {
        ...
        $query = "SELECT fb_user_id FROM #__jfbconnect_user_map " .
                "WHERE j_user_id=" . $dbo->quote($joomlaId);
        $dbo->setQuery($query);
        $facebookId = $dbo->loadResult();
        return $facebookId;
    }
    ...
    function getTotalMappings($includeBlocks = true)
    {
        ...
        if (!$includeBlocks)
        {
            $query .= " INNER JOIN #__users ju ON ju.id=jfbc.j_user_id WHERE ju.block = 0";
        }
        $this->_db->setQuery($query);
    ...
    {
        $query = "UPDATE #__jfbconnect_user_map SET access_token = " . $this->_db->quote($token) .
                " WHERE j_user_id = " . $jUserId;
        $this->_db->setQuery($query);
        $this->_db->query();
    }
