...
class JFormFieldKey extends JFormField
{
  ...
  protected function getInput() 
  {
    ...
    else
    {
      ...
      $button = '<span style="width:5%;">' . JHtml::_('image', 'com_localise/icon-16-arrow.png', '', array('title' => JText::_('COM_LOCALISE_TOOLTIP_TRANSLATION_INSERT'), 'class' => 'hasTip return pointer', 'onclick' => $onclick), true) . '</span>';
  ...
  var translation='" . addslashes(htmlspecialchars($this->element['description'], ENT_COMPAT, 'UTF-8')) . "';translation=translation.replace('%s','___s');translation=translation.replace('%d','___d');translation=translation.replace(/%([0-9]+)\\\$s/,'___\$1');google.language.translate(translation, Localise.language_src, Localise.language_dest, function(result) {if (result.translation) {
      ...
      $button2 .= '<input type="hidden" id="'. $this->id.'text" value="'. $this->element['description']. '" />';
      $button2 .= '<span style="width:5%;">' . JHtml::_('image', 'com_localise/icon-16-bing.png', '', array('title' => JText::_('COM_LOCALISE_TOOLTIP_TRANSLATION_AZURE'), 'class' => 'hasTip translate pointer', 'onclick' => $onclick2, 'rel' => $this->id), true) . '</span>';
    }
    $onkeyup = "javascript:";
