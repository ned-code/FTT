...
class LocaliseModelPackage extends JModelForm
{
  ...
  public function save($data) 
  {
    ...
    //$name = $this->getState('package.name'); //echo 'aaaaaaaaaaaaaaaaaaaaaa'.$name; jexit();
    //if (empty($name)) 
    //{
      $name = $data['name'];
    //}
...
<package>' . "\n";
  ...
  ' . "\n";
  $text.= '
  <manifest client="' . $client . '">' . $manifest . '</manifest>
  ' . "\n";
  $text.= '
  <icon>' . $data['icon'] . '</icon>
  ' . "\n";
...
' . "\n";
    ...
    $translation_model = JModel::getInstance('Translation', 'LocaliseModel', array('ignore_request' => true));
