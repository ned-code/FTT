...
class LocaliseModelTranslation extends JModelForm
{
    ...
    if (!isset($this->item))
    {
      ...
      if (!$this->item)
      {
            ...
            if ($line[0] == '#' || $line[0] == ';')
            {
              ...
              elseif (preg_match('/(#|;)\s*@?(\pL+):?.*/', $line, $matches))
              {
                ...
                case 'copyright':
                  ...
                  if(empty($this->item->additionalcopyright) && $params->get('additionalcopyright'))
                  {
                    $this->item->additionalcopyright[] = $params->get('additionalcopyright');
                  }
                break;
        ...
        if ($this->getState('translation.layout') != 'raw' && empty($this->item->error))
        {
          ...
          //if ($this->item->translated + $this->item->unchanged < $this->item->total) $this->item->complete = false;
        ...
        if ($this->getState('translation.id'))
        {
          ...
          if ($this->item->checked_out == JFactory::getUser()->id) $this->item->checked_out = 0;
        ...
        $extension = substr(str_replace(array('.sys', '.ini'), '', $filename), 6);

        if(empty($this->item->package)){
          $this->item->package = $extension;
        }
        ...
        if(empty($this->item->creationdate)){
          $this->item->creationdate = JFactory::getDate()->format('M Y');
        }
        ...
        if(empty($this->item->license)){
          $this->item->license = $params->get('license');
        }

        if(empty($this->item->description)){
          $languageCode = substr($filename, 0, 5);
          $this->item->description = $extension. ' language '. $filename.'('. $languageCode. ')';
        }
  ...
  protected function preprocessForm(JForm $form, $item, $group = 'content')
  {
    ...
    if ($this->getState('translation.layout') != 'raw')
    {
      ...
      foreach($refsections['sections'] as $section => $value)
      {
        $addform = new JXMLElement('<form />');
        $group = $addform->addChild('fields');
        $group->addAttribute('name', 'sections');
        $subgroup = $group->addChild('fields');
        $subgroup->addAttribute('name', $section);
        $fieldset = $subgroup->addChild('fieldset');
        $fieldset->addAttribute('name', $section);
        $fieldset->addAttribute('label', $section);
        if (isset($sections['sections'][$section]))
        {
          $keys = array_keys(array_merge($value, $sections['sections'][$section]));
        }
        else
        {
        ...
        foreach ($keys as $key)
        {
          ...
          if (!isset($refsections['keys'][$key]))
          {
            $string = $sections['keys'][$key];
            $field->addAttribute('description', $string);
            $field->addAttribute('status', 'extra');
            $field->addAttribute('default', $string);
            $field->addAttribute('label', '<b>'.$key.'</b>');
          }
          elseif (!isset($sections['keys'][$key]))
          {
            $string = $refsections['keys'][$key];
            $field->addAttribute('description', $string);
            $field->addAttribute('status', $this->getState('translation.reference') == $this->getState('translation.tag') ? 'translated' : 'unchanged');
            $field->addAttribute('default', $string);
            $field->addAttribute('label', '<b>'.$key.'</b><br />'.htmlspecialchars($string, ENT_COMPAT, 'UTF-8'));
          }
          else
          {
            $string = $sections['keys'][$key];
            $refstring = $refsections['keys'][$key];
            $field->addAttribute('description', $refstring);
            $field->addAttribute('status', $string == $refstring ? 'unchanged' : 'translated');
            $field->addAttribute('default', $string);
            $field->addAttribute('label', '<b>'.$key.'</b><br />'.htmlspecialchars($refstring, ENT_COMPAT, 'UTF-8'));
          }
          $field->addAttribute('name', $key);
          $field->addAttribute('type', 'key');
          $field->addAttribute('filter', 'raw');
      ...
      foreach($sections['sections'] as $section => $value)
      {
        if (!isset($refsections['sections'][$section]))
        {
          $addform = new JXMLElement('<form />');
          $group = $addform->addChild('fields');
          $group->addAttribute('name', 'sections');
          ...
          foreach($keys as $key)
          {
            if (!isset($refsections['keys'][$key]))
            {
              ...
              $field->addAttribute('type', 'key');
              $field->addAttribute('description', $string);
              $field->addAttribute('reference', $this->getState('translation.reference') == $this->getState('translation.tag') ? 'true' : 'false');
              $field->addAttribute('default', $string);
            }
          }
          $form->load($addform, false);
    ...
    if ($form->getValue('description') == '' && isset($languages[$client][$tag]))
    {
  ...
  public function saveFile($data)
  {
    ...
    {
/*mhehm>*/      $data['description'] = str_replace(array("\r\n", "\n", "\r"), " ", $data['description']);
...
/*<mhehm*/
      ...
      $contents = $contents2.$contents; //mhehm
    }
