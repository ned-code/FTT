...
class JFBConnectModelNotification extends JModel
{
    ...
    {
        $query = "SELECT destination_url, breakout_canvas FROM #__jfbconnect_request r INNER JOIN #__jfbconnect_notification n ON r.id = n.jfbc_request_id " .
                " WHERE n.fb_request_id IN (" . implode(', ', $this->_fbRequestIds) . ") ORDER BY n.created DESC LIMIT 1";
        $this->_db->setQuery($query);
        $redirectInfo = $this->_db->loadObject();
        return $redirectInfo;
    }
}