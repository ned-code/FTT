...
* @version $Id: class_gedcomrecord.php 6602 2009-12-22 18:52:38Z fisharebest $
*/
...
require_once 'class_source.php';
require_once 'class_repository.php';
require_once 'class_note.php';
require_once 'class_media.php';
require_once 'class_event.php';
require_once 'class_serviceclient.php';
...
class GedcomRecord {
	...
	function GedcomRecord($data, $simple=false) {
         //   echo realpath('../components/com_manager/php/gedcom/Parser/ged_config.php');
            if(!$GEDCOM)
...
/*
		...
		//-- set the gedcom record a privatized version
               // var_dump($pgv_lang, $factarray, $GEDCOM, $SHOW_PRIVATE_RELATIONSHIPS, $pgv_private_records, $TBLPREFIX,$global_facts, $person_facts);
		$this->gedrec = privatize_gedcom($this->gedrec,$pgv_lang, $factarray, $GEDCOM, $SHOW_PRIVATE_RELATIONSHIPS, $pgv_private_records, $TBLPREFIX,$global_facts, $person_facts);
	...
	static function &getInstance($data, $simple=true) {
		...
		case 'INDI':
			...
			break;
		case 'FAM':
			$object=new gFamily($data, $simple);
			break;
		case 'SOUR':
			$object=new gSource($data, $simple);
			break;
		//BH ==================
		case 'NOTE':
			$object=new gNote($data, $simple);
			break;
		case 'REPO':
			$object=new gRepository($data, $simple);
			break;
		case 'OBJE':
			$object=new gMedia($data, $simple);
			break;
		default:
	...
	*/
	...
	static function CompareChanDate($x, $y) {
		$chan_x = $x->getChangegEvent();
		$chan_y = $y->getChangegEvent();
	...
	function getAllgEventDates($event) {
		$dates=array();
	...
	function getAllgEventPlaces($event) {
		$places=array();
		foreach ($this->getAllFactsByType($event) as $event) {
	...
	/**
	* Get the first gEvent for the given Fact type
	*
	* @param string $fact
	* @return gEvent
	*/
	...
	* @return gEvent
	*/
	function getChangegEvent() {
		if (is_null($this->changegEvent)) {
			$this->changegEvent = $this->getFactByType('CHAN');
		}
		return $this->changegEvent;
	}
	...
	*/
	function parseFacts($nfacts=NULL) {
		...
		for($i=1; $i<=$lct; $i++) {
			...
			if ($i==$lct||$line{0}==1) {
				if ($i>1){
					$event = new gEvent($factrec, $linenum);
					$fact = $event->getTag();
	...
	*/
	...
	function getgEventDate($event) {
		$srec = $this->getAllgEvents($event);
		if (!$srec) {
			return '';
		}
		$srec = $srec[0];
	...
	function getgEventgSource($event) {
		$srec = $this->getAllgEvents($event);
		if (!$srec) {
			return '';
		}
