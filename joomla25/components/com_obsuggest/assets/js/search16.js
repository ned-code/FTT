...
var Search = {
	...
	displayFalcon_: function(d)
	{
		...
		var height = 0;
		try {
			height = parseInt(this.falcon.getStyle("height"));
		} catch (e) {
			height = parseInt(this.falcon.offsetHeight);
		}
//		var height = parseInt(this.falcon.getStyle("height"));
		if(!d)
		{
			...
			if(height == 0){
				try {
					...
					this.falcon.style.display = 'none';
				}
			} else {
				try {
					this.falcon.setStyle("height", height+"px");
				} catch (e) {
					this.falcon.style.height = height+'px';
				}
	...
	displayFalcon: function(display)
	{
		if(!display)
		{
			$$("div .falcon_container").each(function(el){
...
//			)
	...
	startSearch: function(keyword)
	{
		...
		var req = new Request({
			...
			onComplete: function(txt){
				...
				//tab.scrollTo(0,0)
				Search.iIdeaCount = ($$('#idea_result div.idea-title').length);
				Search.iTop=0;
				Search.setupControl();
	...
	{
		if(!Search.canScroll('down'))	
			$$('#search_control a')[0].className = 'down_disabled';	
		else
			$$('#search_control a')[0].className = 'down';	
		if(!Search.canScroll('up'))	
			$$('#search_control a')[1].className = 'up_disabled';
		else	
			$$('#search_control a')[1].className = 'up';
	},
