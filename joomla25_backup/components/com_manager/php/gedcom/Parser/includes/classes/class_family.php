 ...
 * @version $Id: class_family.php 6578 2009-12-20 09:51:08Z volschin $
 */
...
class gFamily extends GedcomRecord {
	private $husb = null;
	private $wife = null;
	...
	// Create a gFamily object from either raw GEDCOM data or a database row
	function gFamily($data, $simple=true) {
		if (is_array($data)) {
			// Construct from a row from the database

			if ($data['f_husb']) {
				$this->husb=gPerson::getInstance($data['f_husb']);
			}
			if ($data['f_wife']) {
				$this->wife=gPerson::getInstance($data['f_wife']);
			}
		...
		/*	if (preg_match('/^1 HUSB @(.+)@/m', $data, $match)) {
				$this->husb=gPerson::getInstance($match[1], $simple);
			}
                      
			if (preg_match('/^1 WIFE @(.+)@/m', $data, $match)) {
				$this->wife=gPerson::getInstance($match[1], $simple);
			}*/
	...
	// Get an instance of a gFamily.  We either specify
	...
	/**
	 * get the children
	 * @return array 	array of children gPersons
	 */
	...
	function loadChildren() {
		...
		foreach($this->childrenIds as $t=>$chil) {
			$child=gPerson::getInstance($chil);
	...
	function _parseMarriageRecord() {
		$this->marriage = new gEvent(trim(get_sub_record(1, "1 MARR", $this->gedrec)), -1);
	...
	// gNote that this is calculated prior to privatizing the data, so we can
	...
	function getAllMarriageDates() {
		if ($this->canDisplayDetails()) {
			foreach (explode('|', PGV_EVENTS_MARR) as $event) {
				if ($array=$this->getAllgEventDates($event)) {
					return $array;
				}
			}
		}
	...
	function getAllMarriagePlaces() {
		if ($this->canDisplayDetails()) {
			foreach (explode('|', PGV_EVENTS_MARR) as $event) {
				if ($array=$this->getAllgEventPlaces($event)) {
					return $array;
				}
			}
		}
	...
	public function getAllNames() {
		if (is_null($this->_getAllNames)) {
			$husb=$this->husb ? $this->husb : new gPerson('1 SEX M');
			$wife=$this->wife ? $this->wife : new gPerson('1 SEX F');
