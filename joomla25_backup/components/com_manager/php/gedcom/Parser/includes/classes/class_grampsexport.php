 ...
 * @package PhpGedView
 * @subpackage Tools
 * @version $Id: class_grampsexport.php 6578 2009-12-20 09:51:08Z volschin $
 *
 */
...
class GrampsExport {
	...
	var $dom, $ePeople, $eFams, $egSources, $ePlaces, $eObject, $egEvents;
 ...
 */
	...
	function create_event_ref($eParent, $personrec, $event)
	{
		...
		$egEventRef = $this->dom->createElement("eventref");
		$egEventRef = $eParent->appendChild($egEventRef);
		$eventID = $this->create_event($personrec,$event,$eventRec);
		$egEventRef->setAttribute("hlink", $this->query_dom("./events/event[@id = \"$eventID\"]/@handle"));
		$eParent->appendChild($egEventRef);
		}
	}

	/**
	  * Creates the gEvent Element given the record passed and the event abbreviation which is then
	  * 	appended to the parent element passed into the method. When the gEvent element is created
	  ...
	  * @param DOMElement $eventRec - the parent DOMElement to which the created gEvent Element is appended
	  * @param int $done - whether the method is called from the GrampsExport($done=1) or a sub-class
	  */
	...
	*/
	...
	/**
	  * Creates the gNote element and appends it to the parent element
	  *
	  * @param DOMElement $eParent - the parent DOMElement to which the created gNote Element is appended
	  * @param string $noteRec - the entire gFamily, Individual, etc. record in which the event may be found
...
//		}
  ...
  */
	function create_mediaref($eParent, $sourcerefRec, $level,$done=1) {
		...
		$egMediaRef = $this->dom->createElement("objref");
		$egMediaRef = $eParent->appendChild($egMediaRef);
		...
		$egMediaRef->setAttribute("hlink", $this->query_dom("./objects/object[@id = \"$mediaId\"]/@handle"));

		$eParent->appendChild($egMediaRef);
	}
	/**
	...
	function create_media($mediaID, $mediaRec, $level = 1) {
		...
		$num = 1;
		while (($namegSource = get_sub_record($level, $level . " SOUR", $mediaRec, $num)) != null) {
			$this->create_sourceref($object, $namegSource, 1);
			$num++;
		}
	...
	/**
	  * Creates the gSource and appends it to the gSources Element
	  *
	  * @param string $sourceID - the ID of the source to be created
	  * @param string $sourceRec - the entire GEDCOM record containing the gSource
