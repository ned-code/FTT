...
* @version $Id: class_person.php 6593 2009-12-21 21:24:00Z fisharebest $
*/
...
class gPerson extends GedcomRecord {
	var $indifacts = array();
	...
	var $isdead = -1;
	var $sex=null;
	var $generation; // used in some lists to keep track of this gPerson's generation in that list
	...
	// Create a gPerson object from either raw GEDCOM data or a database row
	function gPerson($data, $simple=true) {
		if (is_array($data)) {
	...
	// Get an instance of a gPerson.  We either specify
	...
	static function &getInstance($data, $simple=true) {
		...
		// Create the object
		$object=new gPerson($data, $simple);
		if (!empty($fromfile)) {
	...
	* @return array
	*/
	function findHighlightedgMedia() {
	...
	function getBirthDate() {
		...
		if (is_null($this->_getBirthDate)) {
			if ($this->canDisplayDetails()) {
				...
				$this->_getBirthDate=new GedcomDate("(Private)");
			}
		}
	...
	*/
		...
		if (is_null($this->_getAllBirthDates)) {
			if ($this->canDisplayDetails()) {
				foreach (explode('|', PGV_EVENTS_BIRT) as $event) {
					if ($this->_getAllBirthDates=$this->getAllgEventDates($event)) {
						break;
					}
				}
			} else {
	...
	function getAllBirthPlaces() {
		if (is_null($this->_getAllBirthPlaces)) {
			if ($this->canDisplayDetails()) {
				foreach (explode('|', PGV_EVENTS_BIRT) as $event) {
					if ($this->_getAllBirthPlaces=$this->getAllgEventPlaces($event)) {
						break;
					}
				}
			} else {
		...
		if (is_null($this->_getAllDeathDates)) {
			if ($this->canDisplayDetails()) {
				foreach (explode('|', PGV_EVENTS_DEAT) as $event) {
					if ($this->_getAllDeathDates=$this->getAllgEventDates($event)) {
						break;
					}
				}
			} else {
	...
	function getAllDeathPlaces() {
		if (is_null($this->_getAllDeathPlaces)) {
			if ($this->canDisplayDetails()) {
				foreach (explode('|', PGV_EVENTS_DEAT) as $event) {
					if ($this->_getAllDeathPlaces=$this->getAllgEventPlaces($event)) {
						break;
					}
				}
			} else {
	...
	* @return array array of gFamily objects
	*/
	...
	* @return gPerson  this person's spouse
	*/
	...
	* @return array array of gFamily objects indexed by family id
	*/
	...
	* @return gFamily object
	*/
	function getPrimaryChildgFamily() {
	...
	*/
	function getChildgFamilyPedigree($famid) {
	...
	* @param gFamily $family the family to get the label for
	* @return string
	*/
	function getChildgFamilyLabel(&$family) {
		global $pgv_lang;
		if (!is_null($family)) {
	...
	* @return string
	*/
	function getStepgFamilyLabel(&$family) {
		global $pgv_lang;
		$label = "Unknown gFamily";
		...
		foreach ($childfams as $fam) {
			if (!$fam->equals($family)) {
				...
				if ($label!="Unknown gFamily") return $label;
			}
		}
		return $label;
	}
	...
	* @return string
	*/
	function getSpousegFamilyLabel(&$family) {
	...
	*/
	function parseFacts($nfacts=NULL) {
		...
		if (!$sexfound) {
			$this->globalfacts[] = new gEvent("1 SEX U", 'new');
		}
	}
	/**
	...
	*/
	function add_family_facts($otherfacts = true) {
		...
		foreach ($fams as $famid=>$family) {
			...
			$updfamily = $family->getUpdatedgFamily(); //-- updated family ?
			...
			foreach (array($family->getWife(), $family->getHusband()) as $parent) {
				if ($parent) {
					...
					if ($parent && strstr($SHOW_RELATIVES_EVENTS, $fact)) {
						foreach ($parent->getAllFactsByType(explode('|', PGV_EVENTS_DEAT)) as $sgEvent) {
							$srec = $sgEvent->getGedcomRecord();
							if (GedcomDate::Compare($bDate, $sgEvent->getDate())<0 && GedcomDate::Compare($sgEvent->getDate(), $dDate)<=0) {
								$fact=str_replace('DEAT', $sgEvent->getTag(), $fact); // BURI, CREM, etc.
			...
			foreach ($parents as $indexval=>$parent) {
				...
				if (strstr($SHOW_RELATIVES_EVENTS, $fact)) {
					...
					foreach ($sfamids as $sfamid=>$sfamily) {
						...
						$sgEvent = $sfamily->getMarriage();
						$srec = $sgEvent->getGedcomRecord();
						if (GedcomDate::Compare($bDate, $sgEvent->getDate())<0 && GedcomDate::Compare($sgEvent->getDate(), $dDate)<=0) {
							if ($sfamid==$famid) $fact="_MARR_FAMC";
							$factrec = "1 ".$fact;
							$factrec.="\n".get_sub_record(2, '2 DATE', $srec)."\n".get_sub_record(2, '2 PLAC', $srec);
							if (!$sgEvent->canShow()) $factrec .= "\n2 RESN privacy";
							$factrec .= "\n2 ASSO @".$parent->getXref()."@";
							$factrec .= "\n3 RELA *".$rela;
	...
	* ddate = indi death date record
	*
	* @param string $family gFamily object
	* @param string $option gFamily level indicator
		...
		foreach ($children as $key=>$child) {
			...
			if ($spid!=$except) {
				...
				if (strstr($SHOW_RELATIVES_EVENTS, '_BIRT'.$option)) {
					/* @var $child gPerson */
					/* @var $sgEvent gEvent */
					foreach ($child->getAllFactsByType(explode('|', PGV_EVENTS_BIRT)) as $sgEvent) {
						$srec = $sgEvent->getGedcomRecord();
						$sgdate=$sgEvent->getDate();
						if ($option=='_CHIL' || $sgdate->isOK() && GedcomDate::Compare($this->getEstimatedBirthDate(), $sgdate)<=0 && GedcomDate::Compare($sgdate, $this->getEstimatedDeathDate())<=0) {
							$factrec='1 _'.$sgEvent->getTag().$option;
							$factrec.="\n".get_sub_record(2, '2 DATE', $srec)."\n".get_sub_record(2, '2 PLAC', $srec);
							if (!$sgEvent->canShow()) {
								$factrec.='\n2 RESN privacy';
							}
							if (strstr($srec, "twin")) {
								$rela_sex = gPerson::getInstance($spid)->getSex();
								$rela="twin";
								if ($rela_sex=="F") {
									$rela="twin_sister";
								} elseif ($rela_sex=="M") {
				...
				if (strstr($SHOW_RELATIVES_EVENTS, '_DEAT'.$option)) {
					/* @var $sgEvent gEvent */
					foreach ($child->getAllFactsByType(explode('|', PGV_EVENTS_DEAT)) as $sgEvent) {
						$sgdate=$sgEvent->getDate();
						$srec = $sgEvent->getGedcomRecord();
	...
	* ddate = indi death date record
	*
	* @param string $spouse gPerson object
	...
	*/
	function add_spouse_facts(&$spouse, $famrec="") {
		...
		if ($spouse && strstr($SHOW_RELATIVES_EVENTS, '_DEAT_SPOU')) {
			foreach ($spouse->getAllFactsByType(explode('|', PGV_EVENTS_DEAT)) as $sgEvent) {
				$sdate=$sgEvent->getDate();
				$srec = $sgEvent->getGedcomRecord();
				if ($sdate->isOK() && GedcomDate::Compare($this->getEstimatedBirthDate(), $sdate)<=0 && GedcomDate::Compare($sdate, $this->getEstimatedDeathDate())<=0) {
					$srec=preg_replace('/^1 .*/', "1 _".$sgEvent->getTag()."_SPOU ", $srec);
					$srec.="\n".get_sub_record(2, '2 ASSO @'.$this->xref.'@', $srec);
					$srec.="\n2 ASSO @".$spouse->getXref()."@\n3 RELA *spouse";
					$event = new gEvent($srec, 0);
					$event->setParentObject($this);
					$this->indifacts[] = $event;
				}
			}
	...
	*
	* @param gPerson $spouse Father or mother Gedcom id
	...
	*/
		...
		foreach ($associates as $associate) {
			foreach ($associate->getFacts() as $event) {
				...
				if ($arec) {
					...
					if (!$found) $this->indifacts[] = new gEvent($factrec, 0);
				}
			}
		}
	}
	...
	/**
	* Merge the facts from another gPerson object into this object
	* for generating a diff view
	* @param gPerson $diff the person to compare facts with
	*/
	function diffMerge(&$diff) {
		...
		//-- compare new and old facts of the gPersonal Fact and Details tab 1
		...
		//-- compare new and old facts of the gNotes gSources and gMedia tab 2
		...
		$newfamids = $diff->getChildgFamilyIds();
		if (is_null($this->famc)) $this->getChildgFamilyIds();
		foreach ($newfamids as $id) {
			if (!in_array($id, $this->famc)) $this->famc[]=$id;
		}

		$newfamids = $diff->getSpousegFamilyIds();
		if (is_null($this->fams)) $this->getSpousegFamilyIds();
		foreach ($newfamids as $id) {
	...
	*/
	function getPrimaryParentsNames($classname="", $display="") {
		...
		$fam = $this->getPrimaryChildgFamily();
		if (!$fam) return "";
		$txt = "<div";
	...
	*/
	...
	// Use the SURN field to generate the sortable names.  gNote that this field
	...
	function _addName($type, $full, $gedrec) {
		...
		// Create the list (surname first) version of the name.  gNote that zero
