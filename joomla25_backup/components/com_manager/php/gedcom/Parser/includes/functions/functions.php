 ...
 *
 * @package PhpGedView
 * @version $Id: functions.php 6604 2009-12-22 23:46:51Z yalnifj $
 */
...
require_once '../components/com_manager/php/gedcom/Parser/includes/functions/editlang_edit_settings.php';
require_once '../components/com_manager/php/gedcom/Parser/includes/functions/functions_UTF8.php';


require_once '../components/com_manager/php/gedcom/Parser/includes/functions/functions_DB.php';
require_once '../components/com_manager/php/gedcom/Parser/includes/functions/functions_privacy.php';
require_once '../components/com_manager/php/gedcom/Parser/includes/functions/functions_name.php';
////////////////////////////////////////////////////////////////////////////////
...
function update_site_config($newconfig, $return = false) {
	...
	if (file_exists("../components/com_manager/php/gedcom/Parser/ged_config.php")) {
 ...
 * word wrapped gNotes option.  Routine also avoids splitting UTF-8 encoded
...
function find_highlighted_object($pid, $ged_id, $indirec) {
	...
	if ($ct>0) {
		...
		if (!is_null($client)) {
			...
			for ($i=0; $i<$mt; $i++) {
				$mediaObj = gMedia::getInstance($matches[$i][1]);
...
function compareStrings($aName, $bName, $ignoreCase=true) {
	...
	if ($LANGUAGE == "danish" || $LANGUAGE == "norwegian") {
		...
		$danishTo 	= array("Å", "Å", "Æ", "Æ", "ÿ", "ÿ", "å", "æ", "ø");
	}

	if ($LANGUAGE == "german") {
		$germanFrom = array("AA", "Aa", "Æ", "AE", "Ae", "ÿ", "OE", "Oe", "SS", "Ss", "UE", "Ue", "aa", "æ", "ae", "ø", "oe", "ss", "ue");
...
function sort_facts(&$arr) {
	...
	usort($dated, array("gEvent", "CompareDate"));
	usort($nondated, array("gEvent", "CompareType"));
	...
	while($i<$dc) {
		...
		if ($j<$nc && gEvent::CompareType($dated[$i], $nondated[$j])>0) {
			$arr[$k] = $nondated[$j];
			$j++;
		}
		else {
...
function add_ancestors(&$list, $pid, $children=false, $generations=-1, $show_empty=false) {
	...
	while (count($genlist)>0) {
		...
		$person = gPerson::getInstance($id);
		$famids = $person->getChildFamilies();
		if (count($famids)>0) {
			if ($show_empty) {
				for ($i=0;$i<$num_skipped;$i++) {
					$list["empty" . $total_num_skipped] = new gPerson('');
...
function add_descendancy(&$list, $pid, $parents=false, $generations=-1) {
	$person = gPerson::getInstance($pid);
