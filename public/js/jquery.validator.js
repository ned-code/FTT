...
//								 errorSelector) when it fails validation.
...
(function(jQuery, undefined){
	var options = {
			...
			attributes = {
				type: function( value, type, pass, fail ){
					return ( !value || !types[type] || types[type].test(value) ) ?
						pass() :
				...
				minlength: function( value, attr, pass, fail ) {
					return ( !value || value.length >= attr ) ?
						pass() :
						fail( 'Should be at least ' + attr + ' characters long' ) ;
				},
				...
				maxlength: function( value, attr, pass, fail ) {
					...
					return ( !value || number === -1 || value.length <= number ) ?
						pass() :
						fail( 'Too long. ' + attr + ' characters at most.' );
				},
	...
	function handle(node, options){
		var field = jQuery(node),
				...
				autocomplete = field.attr( 'autocomplete' ) === 'on',
				passFlag = true,
				stopTestFlag = false,
				attr, attrval, rule, response;
		
		for (attr in attributes) {
			attrval = field.attr( attr );
...
(function(jQuery, undefined){
	...
	var cssColors = {
			...
			regex = {
				...
				hslaColor:	/^(?:(hsla)(\())?\s?([0-9]{1,3})\s?,\s?([0-9]{1,3})%\s?,\s?([0-9]{1,3})%\s?,\s?([01](?:\.\d+)?)\s?(\))?$/,
				rgbaColor:	/^(?:(rgba)(\())?\s?([0-9]{1,3})\s?,\s?([0-9]{1,3})\s?,\s?([0-9]{1,3})\s?,\s?([01](?:\.\d+)?)\s?(\))?$/,
			...
			dataTypes = {
				...
				email: function( value, pass, fail ) {
					return jQuery.fn.validator.regex.email.test(value) ?
								 pass() :
								 fail('That doesn\'t look like an email to me') ;
				},
