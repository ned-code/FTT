...
#    @document = Factory.create(:document)
  ...
  context 'update' do
    ...
    before(:each) do
      @default_ub = @document_full_hash[:file]
    end

    it 'should update metadata and increment version if metadata are modified' do
      previous_version = @document.version
      new_metadata = fixture_file('document/default_rdf.rdf')
      @document.media.data = File.open(new_metadata,'rb')
      @document.media.save
      file = File.open(@default_ub,'rb')
      @document.update_with_ub(file, [@document.uuid])
  ...
  context 'delete' do
    ...
    it "should be really deleted" do
      ...
      @document.should be_deleted
      Document.find_by_id(@document.id).should be_nil
      Document.find_by_id(@document.id, :with_deleted => true).should be_nil
  ...
  context 'recently deleted' do
    ...
    it 'should be really deleted after "marked" deleted' do
      @document.destroy.should be_true
      @document = Document.find(@document.id, :with_deleted => true)
  ...
  describe 'collection' do
    ...
    it 'should be retrived with deleted' do
      collection = Document.all(:with_deleted => true)
